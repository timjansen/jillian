    ...
    jelAssert.setCtx(ctx);
      ...
      it('can be created using the JEL syntax', function() {
        ...
        jelAssert.equal('class MyTestType: constructor(x, y): {} a: Number b: String add(): this.a+this.b', 
                        'Class("MyTestType", constructor=(x,y)=>{}, propertyDefs={a: SimpleType("Number"), b: SimpleType("String")}, methods={add: ()=>this.a+this.b})');
      });
      ...
      it('supports properties set in the constructor', function() {
        ...
        jelAssert.equal('with myTestType=class MyTestType: constructor(x: Number, y: String):{}, m=myTestType(y="foo", x=2): [m.x, m.y]', "[2, 'foo']");
      ...
      it('supports superTypes', function() {
        ...
        jelAssert.equal(`with superType=class SuperType: constructor(x: Number, y: Number): {} add(a, b=1):a+b sub(a, b=1):a-b,
                              subType=class SubType extends superType: constructor(x: Number, y: Number, z: Number): {} add(a,b=2):a+b+10 mul(a,b=1):a*b,
                              s1 = superType(4, 6),
                              s2 = subType(8, 10, 20): 
                            [s1.x, s1.y, s2.x, s2.y, s2.z, s1.add(5), s1.sub(4), s2.add(6), s2.sub(33), s2.mul(9, 9)]`, "[4, 6, 8, 10, 20, 6, 3, 18, 32, 81]");
      });
      ...
      it('can be loaded from DB and used like a real type', function() {
        return db.put(ctx, new JEL('class TupleXY: constructor(x: Number, y: Number):{}  add():this.x+this.y static A= 44').executeImmediately(ctx))
