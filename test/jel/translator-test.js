...
describe('jelTranslators', function() {
  ...
  describe('match()', function() {
    ...
    it('should support templates', function() {
        ...
        assert.deepEqual(translator(JEL.createPattern('{{tpl0}}'), JEL.parseTree('5')).match(ctx2, 'a').elements.map(e=>e.value.value), [5]);
        assert.deepEqual(translator(JEL.createPattern('{{tpl0}} {{tpl0}}'), JEL.parseTree('7')).match(ctx2, 'a a').elements.map(e=>e.value.value), [7]);
        assert.deepEqual(translator(JEL.createPattern('{{tpl0}} {{tpl0}}'), JEL.parseTree('7')).match(ctx2, 'b a a').elements.map(e=>e.value.value), []);
        assert.deepEqual(translator(JEL.createPattern('{{tpl0}} {{tpl0}}'), JEL.parseTree('7')).match(ctx2, 'a a b').elements.map(e=>e.value.value), []);
        assert.deepEqual(translator(JEL.createPattern('{{tpl0}} {{tpl0}}'), JEL.parseTree('7')).match(ctx2, 'a b').elements.map(e=>e.value.value), []);
        assert.deepEqual(translator(JEL.createPattern('a {{tpl0}} {{tpl0}} b'), JEL.parseTree('9')).match(ctx2, 'a a a b').elements.map(e=>e.value.value), [9]);

        assert.deepEqual(translator(JEL.createPattern('a [{{tpl0}}]?'), JEL.parseTree('1')).match(ctx2, 'a').elements.map(e=>e.value.value), [1]);
        assert.deepEqual(translator(JEL.createPattern('a [{{tpl0}}]?'), JEL.parseTree('1')).match(ctx2, 'a a').elements.map(e=>e.value.value), [1]);
      
        assert.deepEqual(translator(JEL.createPattern('{{tpl1}}'), JEL.parseTree('7')).match(ctx2, 'a').elements.map(e=>e.value.value), [7]);
        assert.deepEqual(translator(JEL.createPattern('{{tpl1}}'), JEL.parseTree('7')).match(ctx2, 'b').elements.map(e=>e.value.value), [7, 7, 7]);

        assert.deepEqual(translator(JEL.createPattern('{{tpl1}}'), JEL.parseTree('7')).addPattern(JEL.createPattern('{{tpl0}}'), JEL.parseTree('9')).match(ctx2, 'a').elements.map(e=>e.value.value), [7, 9]);
        assert.deepEqual(translator(JEL.createPattern('{{tpl0}}'), JEL.parseTree('7')).addPattern(JEL.createPattern('{{tpl1}}'), JEL.parseTree('9')).match(ctx2, 'a').elements.map(e=>e.value.value), [7, 9]);
        assert.deepEqual(translator(JEL.createPattern('{{tpl0}}'), JEL.parseTree('7')).addPattern(JEL.createPattern('{{tpl1}}'), JEL.parseTree('9')).match(ctx2, 'b').elements.map(e=>e.value.value), [9, 9, 9]);
        assert.deepEqual(translator(JEL.createPattern('{{tpl1}}'), JEL.parseTree('7')).addPattern(JEL.createPattern('{{tpl0}}'), JEL.parseTree('9')).match(ctx2, 'b').elements.map(e=>e.value.value), [7, 7, 7]);
        assert.deepEqual(translator(JEL.createPattern('a {{tpl1}} b'), JEL.parseTree('7')).addPattern(JEL.createPattern('{{tpl0}} {{tpl0}} b'), JEL.parseTree('9')).match(ctx2, 'a a b').elements.map(e=>e.value.value), [7, 9]);
        assert.deepEqual(translator(JEL.createPattern('{{tpl1}} {{tpl1}}'), JEL.parseTree('7')).addPattern(JEL.createPattern('{{tpl0}}'), JEL.parseTree('9')).match(ctx2, 'a a').elements.map(e=>e.value.value), [7]);

        assert.deepEqual(translator(JEL.createPattern('{{t: tpl0 }}'), JEL.parseTree('1')).addPattern(JEL.createPattern('{{t: tpl0}}'), JEL.parseTree('2')).match(ctx2, 'a').elements.map(e=>e.value.value), [1,2]);
        assert.deepEqual(translator(JEL.createPattern('{{t: tpl0 :: t == 2}}'), JEL.parseTree('1')).addPattern(JEL.createPattern('{{t: tpl0 :: t == 1}}'), JEL.parseTree('2')).match(ctx2, 'a').elements.map(e=>e.value.value), [2]);

        assert.deepEqual(translator(JEL.createPattern('{{t: tpl1 }} {{u: tpl0 :: t == 1}}'), JEL.parseTree('t')).addPattern(JEL.createPattern('{{t: tpl0}}'), JEL.parseTree('0')).match(ctx2, 'a a').elements.map(e=>e.value.value), [1]);
        assert.deepEqual(translator(JEL.createPattern('{{t: tpl1 }} {{u: tpl0 :: t == 2}}'), JEL.parseTree('t')).addPattern(JEL.createPattern('{{t: tpl0}}'), JEL.parseTree('0')).match(ctx2, 'b a').elements.map(e=>e.value.value), [2]);
        assert.deepEqual(translator(JEL.createPattern('{{t: tpl1 }} {{u: tpl0 :: t == 1}}'), JEL.parseTree('t')).addPattern(JEL.createPattern('{{t: tpl0}}'), JEL.parseTree('0')).match(ctx2, 'a').elements.map(e=>e.value.value), [0]);
      
        const t0 = translator(JEL.createPattern('{{t: tpl2}}'), JEL.parseTree('t'));
        assert.deepEqual(t0.match(ctx2, 'a').elements.map(e=>e.value.value), [3]);
        assert.deepEqual(t0.match(ctx2, 'a b c').elements.map(e=>e.value.value), [3, 4]);
        assert.deepEqual(t0.match(ctx2, 'd e').elements.map(e=>e.value.value), [5]);
        assert.deepEqual(t0.match(ctx2, 'f a').elements.map(e=>e.value.value), [6]);
        assert.deepEqual(t0.match(ctx2, 'a h').elements.map(e=>e.value.value), [7]);
        assert.deepEqual(t0.match(ctx2, 'a a h').elements.map(e=>e.value.value), [7]);
    });
    ...
    it('should parse real sentences', function() {
        ...
        const ctx2 = ctx.plus({animals, animalSounds, verbs});

        const sounds = translator(JEL.createPattern('the {{animals}} says {{animalSounds}}'), JEL.parseTree('true'));
        assert.deepEqual(sounds.match(ctx2, "the dog says woof").elements.map(e=>e.value.state), [1]);
        assert.deepEqual(sounds.match(ctx2, "the cat says meow").elements.map(e=>e.value.state), [1]);
        assert.deepEqual(sounds.match(ctx2, "the fish says meow").elements.map(e=>e.value.value), []);
        assert.deepEqual(sounds.match(ctx2, "the woof says dog").elements.map(e=>e.value.value), []);
        assert.deepEqual(sounds.match(ctx2, "dog says meow").elements.map(e=>e.value.value), []);
        assert.deepEqual(sounds.match(ctx2, "the cat says meow or something like that").elements.map(e=>e.value.value), []);

        const soundsValidating = translator(JEL.createPattern('the {{a: animals}} says {{s: animalSounds :: s == a}}'), JEL.parseTree('a'));
        assert.deepEqual(soundsValidating.match(ctx2, "the dog says woof").elements.map(e=>e.value.value), ["dog"]);
        assert.deepEqual(soundsValidating.match(ctx2, "the cat says meow").elements.map(e=>e.value.value), ["cat"]);
        assert.deepEqual(soundsValidating.match(ctx2, "the dog says meow").elements.map(e=>e.value.value), []);

        const soundsMeta = translator(JEL.createPattern('the {{a: animals.small}} says {{s: animalSounds :: s == a}}'), JEL.parseTree('true'));
        assert.deepEqual(soundsMeta.match(ctx2, "the dog says woof").elements.map(e=>e.value.state), [1]);
        assert.deepEqual(soundsMeta.match(ctx2, "the cow says woof").elements.map(e=>e.value.value), []);
        assert.deepEqual(soundsMeta.match(ctx2, "the cow says moo").elements.map(e=>e.value.value), []);
        ...
        assert.deepEqual(soundsVerb.match(ctx2, "the dog says woof").elements.map(e=>e.value.value), [1, 2]);
        assert.deepEqual(soundsVerb.match(ctx2, "the dog says moo").elements.map(e=>e.value.value), [1, 2]);
        assert.deepEqual(soundsVerb.match(ctx2, "the cow says moo").elements.map(e=>e.value.value), [1]);
        assert.deepEqual(soundsVerb.match(ctx2, "the dog sleeps").elements.map(e=>e.value.value), [1]);
        assert.deepEqual(soundsVerb.match(ctx2, "the cow sleeps").elements.map(e=>e.value.value), [1]);
        assert.deepEqual(soundsVerb.match(ctx2, "and now something completely different").elements.map(e=>e.value.value), []);
    });
