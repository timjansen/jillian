...
class Parser {
  ...
  tryBinaryOps(left, precedence, stopOps) {
    var reset = this.tokens.mem();
    const binOpToken = this.tokens.next();
    if (!binOpToken)
      return left;
...
//      return this.createBinaryOp(binOpToken.value, left, this.parseExpression(binaryOperators[binOpToken.value], stopOps));
  }
  
  createBinaryOp(opToken, left, right) {
      return {type: 'operator', operator: opToken.value, left, right};
  }
