 ...
 */
export default class Argument extends JelNode implements Serializable {
  constructor(public name: string, public defaultValue?: JelNode | undefined, public type?: JelNode | undefined ) {
    super();
  }

  // override
  execute(ctx: Context): LambdaArgument|Promise<LambdaArgument> {
    if (!this.defaultValue && !this.type)
      return new LambdaArgument(this.name, null, null);
