  ...
  constructor(distributionPoints?: List, public average?: UnitValue|ApproximateNumber|Fraction|number,
	...
	getShare(ctx: Context, value: UnitValue|ApproximateNumber|Fraction|number): Promise<number>|number|null {
		...
		return Util.resolveValues((lt: FuzzyBoolean, gt: FuzzyBoolean)=>{
			...
			let bestShare: any = undefined;
			let ri: number | undefined = undefined;

			return Util.processPromiseList(this.points, r=>JelType.op(ctx, '==', r.value, value), (p, r)=>{
				if (p.toRealBoolean()) 
					bestShare = r.share;
			}, ()=>{
				...
				return Util.processPromiseList(this.points, r=>JelType.op(ctx, '>', r.value, value), (p, r, rstep)=>{
		...
		if (right instanceof Distribution) {
			switch (operator) {
				...
				case '===':
					...
					}, JelType.op(ctx, '===', this.average, right.average));
				case '>>':
				case '>>=':
