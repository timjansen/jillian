	...
	op(ctx: Context, operator: string, right: any): any {
		if (right instanceof Duration) {
			switch (operator) {
				...
				case '<<=':
				case '>>=':
					return JelType.op(ctx, operator, this.typicalSecs, right.typicalSecs);
...
console.log('have to cnv ', right);
			...
			switch (operator) {
				...
				case '==': // simple eq: true if seconds COULD be the same, all worst cases considered (e.g. february, leap years, daylight saving...)
					return FuzzyBoolean.toFuzzyBoolean(value >= this.minSecs && value <= this.maxSecs);
				case '!=': 
					return FuzzyBoolean.toFuzzyBoolean(value < this.minSecs || value > this.maxSecs);
				case '===': // complex eq: only when sure that seconds match
					return FuzzyBoolean.toFuzzyBoolean(fixedSecs == this.minSecs && value == fixedSecs);
				case '!==': 
					return FuzzyBoolean.toFuzzyBoolean(fixedSecs != this.minSecs || value != fixedSecs);
				case '>':
					return FuzzyBoolean.twoPrecision(ctx, this.typicalSecs > value, this.maxSecs > value);
				case '>>':
					return FuzzyBoolean.toFuzzyBoolean(this.maxSecs > value);
				case '<':
					return FuzzyBoolean.twoPrecision(ctx, this.typicalSecs < value, this.minSecs < value);
				case '<<':
					return FuzzyBoolean.toFuzzyBoolean(this.minSecs < value);
				case '>=':
					return FuzzyBoolean.twoPrecision(ctx, this.typicalSecs >= value, this.maxSecs >= value);
				case '>>=':
					return FuzzyBoolean.toFuzzyBoolean(this.maxSecs >= value);
	...
	private static minMax(ctx: Context, operator: string, args: any[]): Duration {
		...
		for (let d of args) {
			...
			if (JelType.op(ctx, operator, l, dur).toRealBoolean())
				l = dur;
		}
	 return l;
	}
	...
	static create(ctx: Context, ...args: any[]): any {
		if (args[0] instanceof UnitValue) {
			...
			if (uv.isType(ctx, 'Year').toRealBoolean())
				return new Duration(JelType.toNumber(uv));
			else if (uv.isType(ctx, 'Month').toRealBoolean())
				return new Duration(0, JelType.toNumber(uv));
			else if (uv.isType(ctx, 'Day').toRealBoolean())
				return new Duration(0, 0, JelType.toNumber(uv));
			else if (uv.isType(ctx, 'Hour').toRealBoolean())
				return new Duration(0, 0, 0, JelType.toNumber(uv));
			else if (uv.isType(ctx, 'Minute').toRealBoolean())
				return new Duration(0, 0, 0, 0, JelType.toNumber(uv));
			else if (uv.isType(ctx, 'Second').toRealBoolean())
				return new Duration(0, 0, 0, 0, 0, JelType.toNumber(uv));
			else
				return Util.resolveValue(uv.convertTo(ctx, 'Second'), r=>new Duration(0, 0, 0, 0, 0, JelType.toNumber(r)).simplify());
		}
